...
//--------------------------------------------------------------------------------------------------//
    ...
    let gridPositions (resolution, minX, maxX, minY, maxY) =
        let xPositions = GenerateData.fromList [minX .. resolution .. maxX]
        let yPositions = GenerateData.fromList [minY .. resolution .. maxY]
        GenerateData.product (fun x y -> middleOfSheet + { X = float x; Y = float y }) xPositions yPositions
    ...
    let makeTest1Circuit (andPos:XYPos) =
        ...
        // Apply createCircuit to each position pair generated by filteredGenPositions.
        // Depending on your setup, you may map over the generator, run it a certain number of times, etc.
    let makeWireRoutingTestCircuit (andPos:XYPos, andRotation:Rotation, andFlip:SymbolT.FlipType option, dffRotation:Rotation,dffFlip:SymbolT.FlipType option) =
        initSheetModel
        |> placeSymbol "G1" (GateN(And,2)) andPos
        |> Result.bind (placeSymbol "FF1" DFF middleOfSheet)
        |> Result.map (rotateSymbol "G1" andRotation)
        |> Result.map ( match andFlip with
                                | Some flip -> flipSymbol "G1" flip
                                | None -> id)
        |> Result.map ( rotateSymbol "FF1" dffRotation)
        |> Result.map ( match andFlip with
                        | Some flip -> flipSymbol "G1" flip
                        | None -> id)
        |> Result.bind (placeWire (portOf "FF1" 0) (portOf "G1" 0) )
...
//------------------------------------------------------------------------------------------------//
    ...
    module Asserts =
        ...
        let failOnWireNotSymbolIntersectsSymbol (sample: int) (sheet: SheetT.Model) =
            let wireFailure = failOnWireIntersectsSymbol sample sheet
            let symbolFailure = failOnSymbolIntersectsSymbol sample sheet

            if symbolFailure.IsNone then
                wireFailure
            else
                None
...
//---------------------------------------------------------------------------------------//
    ...
    module Tests =
        ...
        let test5 testNum firstSample dispatch =
            runTestOnSheets
                "Test for automatic wire routing"
                firstSample
                (gridPositions(20, -100, 100, -100, 100))     
                makeTest1Circuit
                Asserts.failOnWireNotSymbolIntersectsSymbol
                dispatch
            |> recordPositionInTest testNum dispatch
